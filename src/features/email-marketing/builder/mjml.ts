...
export function blockToMjml(block: BlockData, gs: Record<string, any> = {}, preserveTags = false, customData?: Record<string, string>): string {
  ...
  switch (type) {
    ...
    /* ── Order Details ── */
    case 'order_details': {
      const itemsTag = replaceMergeTags('{{order_items_table}}', preserveTags, customData);
      const breakdownTag = replaceMergeTags('{{order_price_breakdown}}', preserveTags, customData);
      return `<mj-section padding="${ps}"><mj-column width="100%"><mj-text padding="0" font-family="${font}">${itemsTag}</mj-text><mj-divider border-width="1px" border-color="#e5e7eb" padding="12px 0" /><mj-text padding="0" font-family="${font}">${breakdownTag}</mj-text></mj-column></mj-section>`;
    }
